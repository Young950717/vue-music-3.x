    ...
    <div class="bg-image" ref="bgImage" :style="bgImageStyle">
      <div class="play-btn-wrapper" :style="playBtnStyle">
        <div v-show="songs.length > 0" class="play-btn">
          <i class="icon-play"></i>
          <span @click="random" class="text">随机播放全部</span>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    filterStyle () {
      const blur = this.scrollY >= 0 ? Math.min(this.maxTranslateY / this.imageHeight, this.scrollY / this.imageHeight) * 20 : 0
      return {
    ...
    playBtnStyle () {
      return {
        display: this.scrollY >= this.maxTranslateY ? 'none' : ''
      }
    }
  },
  mounted () {
